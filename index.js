...
app.post("/api/create-checkout", async (c) => {
  // Try env vars first, then Railway-specific env access
  const stripeSecret = process.env.STRIPE_SECRET_KEY 
    || process.env['STRIPE_SECRET_KEY']
    || Bun.env.STRIPE_SECRET_KEY;
  const priceId = process.env.STRIPE_PRICE_ID
    || process.env['STRIPE_PRICE_ID'] 
    || Bun.env.STRIPE_PRICE_ID;
  const basicPriceId = process.env.STRIPE_BASIC_PRICE_ID
    || Bun.env.STRIPE_BASIC_PRICE_ID;
  if(!stripeSecret || !priceId) {
    return c.json({ 
      ...
      debug: {
        ...
        hasPriceId: !!priceId,
        bunEnvKeys: Object.keys(Bun.env).filter(k => k.includes('STRIPE'))
      }
    }, 500);
  }
  const basicPriceId = process.env.STRIPE_BASIC_PRICE_ID;
  try {
    ...
    const selectedPrice = (plan === 'basic' && basicPriceId) ? basicPriceId : priceId;
...
app.post("/api/verify-subscription", async (c) => {
  ...
  try {
    ...
    const priceUsed = sub.items?.data?.[0]?.price?.id;
...
app.get("/debug", (c) => {
  const key = process.env.ANTHROPIC_API_KEY || Bun.env.ANTHROPIC_API_KEY;
  const stripe = process.env.STRIPE_SECRET_KEY || Bun.env.STRIPE_SECRET_KEY;
  const priceId = process.env.STRIPE_PRICE_ID || Bun.env.STRIPE_PRICE_ID;
  const basicPriceId = process.env.STRIPE_BASIC_PRICE_ID || Bun.env.STRIPE_BASIC_PRICE_ID;
  // Show ALL env keys to find where Stripe vars went
  const allKeys = Object.keys(process.env);
  return c.json({
    hasAnthropicKey: !!key,
    hasStripeSecret: !!stripe,
    stripeStart: stripe ? stripe.slice(0,12) : "NOT FOUND",
    hasPriceId: !!priceId,
    hasBasicPriceId: !!basicPriceId,
    totalEnvVars: allKeys.length,
    allEnvKeys: allKeys,
    bunEnvKeys: Object.keys(Bun.env),
    hasStripeInBun: !!Bun.env.STRIPE_SECRET_KEY
  });
});
