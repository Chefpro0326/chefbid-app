...
<div id="authScreen">
  ...
  <div class="auth-box">
    ...
    <div id="signupForm" style="display:none">
      ...
      <button class="auth-btn" onclick="doSignup()" id="signupBtn">Create Account →</button>
    </div>
    <div id="loginApiWrap"></div><div class="auth-error" id="authError"></div>
    <div class="auth-pro-badge">✦ PRO PLAN · All Features Unlocked</div>
  </div>
</div>
...
<div id="appShell">
  ...
  <div class="sidebar">
    ...
    <div class="sidebar-footer">
      <div style="font-size:0.68rem;color:rgba(255,255,255,0.2);padding:8px 4px;font-family:'DM Mono',monospace">✦ AI Powered by ChefBid</div>
  ...
  <div class="main-content">
    ...
    <div class="page" id="page-newEvent">
      ...
      <!-- Ingredients -->
      <div class="mono-label">MENU & INGREDIENTS</div>
      <div class="card">
        <div class="card-head">🍽️ Describe Your Menu
          <span style="font-size:0.72rem;font-family:'DM Mono',monospace;color:var(--gold);font-weight:400;margin-left:8px">AI auto-generates all ingredients + quantities</span>
        </div>

        <!-- AI Menu Description -->
        <div class="field">
          <label>Describe your menu or paste a recipe</label>
          <textarea id="evRecipe" rows="4" placeholder="e.g. 3-course plated dinner: Caesar salad, pan-seared salmon with roasted asparagus and garlic mashed potatoes, chocolate lava cake. Full service." style="min-height:90px"></textarea>
        </div>
        <button class="btn-gold btn-sm" onclick="aiParseMenu()" id="parseMenuBtn" style="width:100%;margin-bottom:16px">
          ✦ AI — Auto-Generate Ingredients &amp; Portions
        </button>
        <div id="parseMenuStatus" style="display:none;text-align:center;font-size:0.8rem;color:var(--gold);font-family:'DM Mono',monospace;margin-bottom:12px">
          ⏳ AI is calculating portions...
        </div>

        <!-- Divider -->
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
          <div style="flex:1;height:1px;background:var(--border)"></div>
          <span style="font-size:0.65rem;color:var(--ink-dim);font-family:'DM Mono',monospace;letter-spacing:1px">OR ADD MANUALLY</span>
          <div style="flex:1;height:1px;background:var(--border)"></div>
        </div>

        <!-- Manual ingredient rows -->
        <div id="ingRows"></div>
        <button class="add-row-btn" onclick="addIng()">+ Add Ingredient Manually</button>
      </div>
...
function doSignup() {
  ...
  users[email] = { name, email, pw, created: new Date().toISOString(), plan: 'pro' };
  DB.set('users', users);
...
async function lookupVendorPrices() {
  ...
  if(!vendors.length) { showVendorError('Please enter at least one vendor name.'); return; }

  const btn = document.getElementById('vendorLookupBtn');
  btn.disabled = true; btn.textContent = '⏳ Looking up prices...';

  // Build ingredient name list — prefer AI-parsed ingredients
  let ingNames;
  if(window._parsedIngredients && window._parsedIngredients.length) {
    ingNames = window._parsedIngredients.map(i => i.name).slice(0, 15);
  } else if(ings.length) {
    ingNames = ings.map(i => i.split(':')[0].trim()).slice(0, 15);
  } else {
    ingNames = recipe.split(/[,\\n]/).map(s => s.trim()).filter(Boolean).slice(0, 15);
  }

  const vendorList = vendors.join(', ');
  const ingList = ingNames.join(', ');

  const prompt = \`You are a US food wholesale pricing expert. Return realistic 2025 wholesale price estimates.

VENDORS: \${vendorList}
INGREDIENTS: \${ingList}

STRICT OUTPUT RULES - YOUR RESPONSE MUST:
- Start with { and end with }
- Contain ONLY valid JSON
- Have NO markdown, NO backticks, NO code fences, NO explanation

Use this EXACT structure:
{"ingredients":["ingredient1","ingredient2"],"vendors":["vendor1"],"prices":{"ingredient1":{"vendor1":{"price":2.50,"unit":"lb"}},"ingredient2":{"vendor1":{"price":1.20,"unit":"lb"}}},"bestVendor":"vendor1","bestVendorReason":"lowest overall prices","savingsNote":"estimated 10% savings"}

Now output the JSON for the ingredients and vendors listed above:\`;
  ...
  try {
    ...
    if (data.error) { showVendorError('API error: ' + (data.error.message || JSON.stringify(data.error))); btn.disabled=false; btn.textContent='🔍 AI Lookup Prices for My Ingredients'; return; }
    ...
    console.log('Raw after strip:', raw.slice(0, 200));

    // Find the JSON object - from first { to last }
    const start = raw.indexOf('{');
    const end = raw.lastIndexOf('}');
    if (start === -1 || end === -1) {
      showVendorError('No JSON found. Response: ' + raw.slice(0,200));
      btn.disabled=false; btn.textContent='🔍 AI Lookup Prices for My Ingredients';
      return;
    }

    const jsonStr = raw.slice(start, end + 1);
    console.log('JSON to parse:', jsonStr.slice(0, 200));

    let parsed;
    try {
      parsed = JSON.parse(jsonStr);
    } catch(e) {
      console.error('Parse error:', e.message, 'JSON was:', jsonStr.slice(0,300));
      showVendorError('Parse error: ' + e.message);
      btn.disabled=false; btn.textContent='🔍 AI Lookup Prices for My Ingredients';
      return;
    }

    if (!parsed.ingredients || !parsed.prices) {
      showVendorError('Unexpected response structure from AI.');
      btn.disabled=false; btn.textContent='🔍 AI Lookup Prices for My Ingredients';
      return;
    }

    // Store and display results
    console.log('Vendor data parsed:', JSON.stringify(parsed).slice(0,500));
    aiVendorPrices = parsed;
    // Show the results container
    const resultBox = document.getElementById('vendorPriceResults');
    if(resultBox) { resultBox.style.display = 'block'; console.log('Results box shown'); }
    else { console.error('vendorPriceResults element not found!'); }
    // Render the comparison table
    console.log('About to call renderVendorTable, type:', typeof renderVendorTable);
    try {
      renderVendorTable(parsed, vendors);
      console.log('renderVendorTable called successfully');
    } catch(renderErr) {
      console.error('renderVendorTable error:', renderErr.message);
      // Fallback: show raw data directly
      if(resultBox) {
        resultBox.style.display = 'block';
        resultBox.innerHTML = '<div style="padding:16px"><div style="font-weight:700;margin-bottom:12px;color:var(--ink)">✓ Vendor Prices Loaded</div>' +
          '<table style="width:100%;border-collapse:collapse;font-size:0.85rem">' +
          '<tr style="background:var(--ink);color:#fff"><th style="padding:8px;text-align:left">Ingredient</th>' +
          vendors.map(v => \`<th style="padding:8px;text-align:right">\${v}</th>\`).join('') + '</tr>' +
          (parsed.ingredients||[]).map(ing => {
            const prices = parsed.prices[ing] || {};
            return '<tr style="border-bottom:1px solid var(--border)"><td style="padding:8px;font-weight:600">' + ing + '</td>' +
              vendors.map(v => {
                const info = prices[v] || prices[v.toLowerCase()] || {};
                return '<td style="padding:8px;text-align:right;font-family:DM Mono,monospace">' + (info.price ? '$'+info.price.toFixed(2)+'/'+info.unit : '—') + '</td>';
              }).join('') + '</tr>';
          }).join('') +
          '</table>' +
          (parsed.bestVendor ? \`<div style="margin-top:12px;padding:10px;background:#ECFDF5;border-radius:8px;font-size:0.85rem"><strong style="color:#059669">✓ Best: \${parsed.bestVendor}</strong> — \${parsed.bestVendorReason||''}</div>\` : '') +
          '</div>';
      }
    }
    showToast('✓ Vendor prices loaded for ' + (parsed.ingredients||[]).length + ' ingredients!');

  } catch(err) {
    if (err.name === 'AbortError') {
      showVendorError('Request timed out. Please try again.');
    } else {
      showVendorError('Error: ' + err.message);
    }
  } finally {
    btn.disabled = false;
...
function renderVendorTable(data, vendors) {
  console.log('renderVendorTable v2 called, ingredients:', (data.ingredients||[]).length);

  // Force show the results container and all parents
  const resultBox = document.getElementById('vendorPriceResults');
  if(resultBox) {
    resultBox.style.display = 'block';
  ...
  // Build complete table HTML and inject directly
  ...
  html += '<th style="text-align:left;padding:10px 12px;color:rgba(255,255,255,0.6);font-family:DM Mono,monospace;font-size:0.68rem;letter-spacing:1px;text-transform:uppercase">INGREDIENT</th>';
  vendors.forEach(v => {
    html += \`<th style="text-align:right;padding:10px 12px;color:#d4a020;font-family:DM Mono,monospace;font-size:0.68rem;letter-spacing:1px;text-transform:uppercase">\${v}</th>\`;
  });
  html += '<th style="text-align:right;padding:10px 12px;color:#52D9A8;font-family:DM Mono,monospace;font-size:0.68rem;letter-spacing:1px;text-transform:uppercase">BEST</th>';
  html += '</tr></thead><tbody>';

  // Data rows
  ingredients.forEach((ing, i) => {
    const ingPrices = prices[ing] || {};
    let bestPrice = Infinity, bestVendor = '';

    // Pre-calculate best price
    vendors.forEach(v => {
      const info = ingPrices[v] || ingPrices[v.toLowerCase()] || {};
    ...
    vendors.forEach(v => {
      ...
      if(p) {
        ...
        html += \`<td style="text-align:right;padding:8px 12px;color:#9ca3af;font-size:0.85rem">—</td>\`;
      }
    });
  ...
  // Inject directly into resultBox
  if(resultBox) {
    resultBox.innerHTML = '<div class="mono-label">AI PRICE COMPARISON</div>' + html;
    resultBox.style.display = 'block';
    console.log('Table injected, resultBox innerHTML length:', resultBox.innerHTML.length);
  }
}
...
// ═══════════════════════════════════════════════
// API PROXY — key stored securely on server
function apiHeaders() {
...
// AI MENU PARSER
// ═══════════════════════════════════════════════
async function aiParseMenu() {
  const recipe = document.getElementById('evRecipe').value.trim();
  const guests = parseInt(document.getElementById('evGuests').value) || 0;
  if (!recipe) { showToast('Please describe your menu first!'); return; }
  if (!guests || guests < 1) { showToast('Please enter number of guests first!'); return; }

  const btn = document.getElementById('parseMenuBtn');
  const status = document.getElementById('parseMenuStatus');
  btn.disabled = true;
  btn.textContent = '⏳ AI Calculating...';
  ...
  try {
    const res = await fetch('/api/ai', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: \`You are a professional catering chef. Calculate exact ingredients for \${guests} guests with 10% waste buffer.

MENU: \${recipe}

Use professional catering portions (6oz protein, 4oz starch, 3oz vegetable per person).
Include ALL ingredients: proteins, starches, vegetables, sauces, garnishes, bread, butter, etc.

Respond ONLY with raw JSON, no markdown, no explanation:
{"ingredients":[{"name":"Salmon Fillet","portionPerPerson":6,"unit":"oz"},{"name":"Asparagus","portionPerPerson":3,"unit":"oz"}]}\`
        }]
      })
    });
    const data = await res.json();
    ...
    // Calculate totals and populate compact ingredient list
    document.getElementById('ingRows').innerHTML = '';

    // Build compact summary table instead of individual input rows
    const tableHTML = \`
      <div style="background:var(--surface);border-radius:12px;overflow:hidden;border:1px solid var(--border);margin-bottom:10px">
        <div style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:0;background:var(--ink);padding:8px 12px">
          <div style="font-size:0.65rem;color:rgba(255,255,255,0.5);font-family:'DM Mono',monospace;letter-spacing:1px;text-transform:uppercase">INGREDIENT</div>
          <div style="font-size:0.65rem;color:rgba(255,255,255,0.5);font-family:'DM Mono',monospace;letter-spacing:1px;text-transform:uppercase">PER PERSON</div>
          <div style="font-size:0.65rem;color:var(--gold);font-family:'DM Mono',monospace;letter-spacing:1px;text-transform:uppercase">TOTAL (\${guests} guests)</div>
          <div></div>
        </div>
        <div id="ingTableBody">
          \${parsed.ingredients.map((ing, idx) => {
            const total = (ing.portionPerPerson * guests * 1.1).toFixed(1);
            // Convert oz to lbs if over 16oz
            let displayTotal = total;
            let displayUnit = ing.unit;
            if(ing.unit === 'oz' && parseFloat(total) >= 16) {
              displayTotal = (parseFloat(total)/16).toFixed(1);
              displayUnit = 'lbs';
            }
            return \`<div style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:0;padding:8px 12px;border-top:1px solid var(--border);align-items:center" id="ing-row-\${idx}">
              <div style="font-weight:600;font-size:0.88rem">\${ing.name}</div>
              <div style="font-size:0.82rem;color:var(--ink-dim)">\${ing.portionPerPerson} \${ing.unit}</div>
              <div style="font-family:'DM Mono',monospace;font-size:0.88rem;color:var(--ink);font-weight:700">\${displayTotal} \${displayUnit}</div>
              <button onclick="removeIngRow(\${idx})" style="background:none;border:none;color:var(--ink-dim);cursor:pointer;font-size:0.9rem;padding:2px 6px">×</button>
            </div>\`;
          }).join('')}
        </div>
      </div>
      <div style="font-size:0.72rem;color:var(--ink-dim);font-family:'DM Mono',monospace;margin-bottom:8px">
        ✓ \${parsed.ingredients.length} ingredients · \${guests} guests · 10% waste buffer included
      </div>\`;

    document.getElementById('ingRows').innerHTML = tableHTML;

    // Store parsed ingredients for vendor lookup and scaling
    window._parsedIngredients = parsed.ingredients.map(ing => ({...ing, guests}));

    // Also add hidden input rows for the scaling engine
    parsed.ingredients.forEach(ing => {
      const r = document.createElement('div');
      r.className = 'row-grid ing-grid';
      r.style.display = 'none';
      r.innerHTML = \`
        <div class="field"><input type="text" value="\${ing.name.replace(/"/g,'&quot;')}" oninput="scheduleScaling()"></div>
        <div class="field"><input type="number" value="\${ing.portionPerPerson}" step="0.1" min="0" oninput="scheduleScaling()"></div>
        <div class="field"><select onchange="scheduleScaling()">
          \${['oz','lbs','kg','cups','each','gallons','dozen','cases'].map(u => \`<option\${u===ing.unit?' selected':''}>\${u}</option>\`).join('')}
        </select></div>
        <div class="field"><button class="icon-btn del-btn" onclick="this.closest('.row-grid').remove();scheduleScaling()">×</button></div>\`;
      document.getElementById('ingRows').appendChild(r);
    });

    scheduleScaling();
    showToast('✦ ' + parsed.ingredients.length + ' ingredients calculated for ' + guests + ' guests!');
    document.getElementById('ingRows').scrollIntoView({ behavior: 'smooth', block: 'start' });
    ...
    btn.disabled = false;
    btn.innerHTML = '✦ AI — Auto-Generate Ingredients &amp; Portions';
...
function removeIngRow(idx) {
  ...
  if(row) row.remove();
  // Also remove hidden scaling row
  const hiddenRows = document.querySelectorAll('#ingRows .row-grid.ing-grid');
  if(hiddenRows[idx]) hiddenRows[idx].remove();
  scheduleScaling();
}
...
async function askFollowUp() {
  ...
  try {
    const fuHeaders = apiHeaders();
    const res = await fetch("/api/ai", {
      method:"POST", headers:fuHeaders,
      body: JSON.stringify({ model:"claude-sonnet-4-5", max_tokens:1000, messages:[{role:"user",content:\`ChefBid AI. Context: \${ctx}\\n\\nChef asks: \${q}\\n\\nAnswer concisely, chef-to-chef, with specific numbers.\`}] })
    });
    const data = await res.json();
    const text = data.content?.map(i=>i.text||'').join('\\n')||'Unable to respond.';
...
function clearForm() {
  ...
  document.getElementById('ingRows').innerHTML='';
  // Keep vendor results visible
  // aiVendorPrices = []; // Don't reset vendor prices
  addIng(); addIng(); addIng();
...
const PLANS = {
  basic: { priceId: 'price_1TLcNkHpRjFN2n0m2llx9Mi3', name: 'Basic', amount: 19 },
  pro:   { priceId: 'price_1TORLzHpRjFN2n0mqlRKR0EB', name: 'PRO',   amount: 29 }
};
...
async function startCheckout(plan) {
  ...
  try {
    ...
    const result = await stripe.redirectToCheckout({
      lineItems: [{ price: p.priceId, quantity: 1 }],
      mode: 'subscription',
    ...
    if (result.error) showToast('Payment error: ' + result.error.message);
  } catch(err) {
...
(function() {
  ...
  if (params.get('payment') === 'success') {
    ...
    showToast('🎉 Payment successful! Welcome to ChefBid PRO!');
...
console.log("ChefBid PRO v3 EMBEDDED running on port " + server.port);
