...
app.get("/debug", (c) => {
  ...
  return c.json({
    hasAnthropicKey: !!key,
    anthropicStart: key ? key.slice(0,12) : "NOT FOUND",
    hasStripeSecret: !!stripe,
    stripeStart: stripe ? stripe.slice(0,12) : "NOT FOUND",
    hasPriceId: !!priceId,
    priceIdStart: priceId ? priceId.slice(0,15) : "NOT FOUND",
    hasBasicPriceId: !!basicPriceId,
    basicPriceIdStart: basicPriceId ? basicPriceId.slice(0,15) : "NOT FOUND",
    allEnvKeys: Object.keys(process.env).filter(k => k.includes('STRIPE') || k.includes('ANTHROPIC'))
  });
});
